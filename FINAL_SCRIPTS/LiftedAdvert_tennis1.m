...
tform_net = estimateGeometricTransform(net_rect,net_dist_rect,'projective');

% define the final transformation (projective*affine)
tform1 = projective2d(tform_translation1.T*tform_net.T);
tform2 = projective2d(tform_translation2.T*tform_net.T);
...
video_out = VideoWriter('output_video');
open(video_out);
